...
def rotation_matrix_to_quaternion(R):
    ...
    Args:
        R: A tensor of shape (3, 3) representing the rotation matrix.

    Returns:
        A tensor of shape (4,) representing the quaternion [w, i, j, k].
    """
    R = R.cpu()
    m = R.numpy()  # Convert to numpy array for easier manipulation
...
def extrinsic_to_6dof(extrinsic):
    """
    Convert a 3x4 extrinsic matrix to a 6DoF parameterization.
    
    Args:
        extrinsic: A torch tensor of shape (1, 3, 12) representing the extrinsic matrix.

    Returns:
        A tensor of shape (1, 3, 7) representing the 6DoF parameterization [x, y, z, w, i, j, k].
    """
    batch_size, _, _ = extrinsic.shape
    extrinsic = extrinsic.view(batch_size, 3, 4)
    ...
    for i in range(batch_size):
        quaternion[i] = rotation_matrix_to_quaternion(R[i])
    
    quaternion = quaternion.to(t.device)
    
    # Combine translation vector and quaternion
    translation_quaternion = torch.cat((t, quaternion), dim=1)
    
    return translation_quaternion
