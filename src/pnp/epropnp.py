...
class EProPnPBase(torch.nn.Module, metaclass=ABCMeta):
    ...
    def monte_carlo_forward(self, x3d, x2d, w2d, camera, cost_fun,
        ...
        Args:
            ...
            x2d (Tensor): Shape (num_obj, num_points, 2) -> Moi object 1 ảnh thôi, mục tiêu là từ 1 ảnh reconstruct 1 object 3D mà. 
            w2d (Tensor): Shape (num_obj, num_points, 2)
            camera: Camera object of batch size (num_obj, ) -> Mỗi object 1 camera. 
        ...
        cost_init = evaluate_fun(pose=pose_init)[1] if pose_init is not None else None # pose_init chinh la pose ground_truth.
        ...
        pose_opt, pose_cov, cost, pose_opt_plus = self.solver(
            ...
            normalize_override=False, **kwargs) # Hết đoạn này là biết được pose rồi, đoạn sau dùng để tính loss. 
        ...
        if num_obj > 0:
            ...
            # Algorithm 1. But not return L_pred
