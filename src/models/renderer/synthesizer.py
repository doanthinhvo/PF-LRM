...
class TriplaneSynthesizer(nn.Module):
    ...
    def forward(self, planes, cameras, render_size=128, crop_params=None, nerf_supervised=False, render_size_nerf_supervised=None ):
        # planes: (N, 3, D', H', W')
        # cameras: (N, M, D_cam)
        # render_size: int
        assert planes.shape[0] == cameras.shape[0], "Batch size mismatch for planes and cameras"
        N, M = cameras.shape[:2] # N: bs, M num views, because of we render 1 frame at a time so 1, 1 
        ...
        if nerf_supervised:
            ray_origins_2, ray_directions_2 = self.ray_sampler(
                cam2world_matrix=cam2world_matrix.reshape(-1, 4, 4),
                intrinsics=intrinsics.reshape(-1, 3, 3),
                render_size=render_size_nerf_supervised,
            )
        ...
        # ray for NeRF supervision. 
        if nerf_supervised:
            ray_origins_2 = ray_origins_2.reshape(N*M, render_size_nerf_supervised, render_size_nerf_supervised, 3)
            ray_directions_2 = ray_directions_2.reshape(N*M, render_size_nerf_supervised, render_size_nerf_supervised, 3)
            central_pixels = get_central_point_of_patch(render_size_nerf_supervised, patch_size=16)
                        
            _ray_origins_2 = []
            _ray_directions_2  = [] 
            for i, j in central_pixels:
                _ray_origins_2.append(ray_origins_2[:,i,j,:])
                _ray_directions_2.append(ray_directions_2[:,i,j,:])

            ray_origins_2 = torch.stack(_ray_origins_2, dim=1)
            ray_directions_2 = torch.stack(_ray_directions_2, dim=1)

        # Perform volume rendering for L_C loss
        rgb_samples, depth_samples, weights_samples, point_samples, tau = self.renderer(
            planes.repeat_interleave(M, dim=0), self.decoder, ray_origins, ray_directions, self.rendering_kwargs,
        )

        # Perform volume rendering for Loss NeRF supervision
        point_samples_2 = None
        tau_2 = None
        if nerf_supervised:
            rgb_samples_2, depth_samples_2, weights_samples_2, point_samples_2, tau_2 = self.renderer(
                planes.repeat_interleave(M, dim=0), self.decoder, ray_origins_2, ray_directions_2, self.rendering_kwargs,
            )
        ...
        # NeRF Supervision
        if point_samples_2 is None and tau_2 is None:
            point_samples_2 = point_samples # 1, 400, 3
            tau_2 = tau # 1, 400, 1
        point_samples_2 = point_samples_2.reshape(N, M, Himg*Wimg, 3)
        tau_2 = tau_2.reshape(N, M, Himg*Wimg, 1)
        
        out = {
            'images_rgb': rgb_images,
            'images_depth': depth_images,
            'images_weight': weight_images,
            'triplane_point_samples': point_samples_2,
            'tau': tau_2,
        }
        return out
