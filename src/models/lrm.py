...
from .decoder.point_estimator import PointMLP
from ..utils.mesh_util import xatlas_uvmap

    
class PF_LRM(nn.Module):
    """
    ...
    def __init__(
        ...
        self.pointmlp = PointMLP(input_dim=encoder_feat_dim, output_dim=3, hidden_dim=512, num_layers=4)
    ...
    def forward_planes(self, images, cameras):
        ...
        # encode images
        image_feats = self.encoder(images, cameras) # TODO: Fix this
        image_feats = rearrange(image_feats, '(b v) l d -> b (v l) d', b=B)
        
        # transformer generating planes
        planes, images_feats = self.transformer(image_feats)
        
        return planes, images_feats
